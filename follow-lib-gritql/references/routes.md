...
- Metavariables: `$x`, `$$$args` (spread), `$_` (anonymous)
- Conditions: `where`, `if`, `and`, `or`, `not`, `contains`, `within`, `includes`
- Biome plugin: วาง `.grit` file ใน project → ใช้เป็น lint rule
- Rewrite: `` `old` => `new` `` สำหรับ autofix
