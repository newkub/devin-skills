...
สกัด workflow หรือการทำงานที่เกิดขึ้นใน chat session ปัจจุบัน แล้วสร้างเป็น skill ใหม่ใน `%APPDATA%\devin\skills` ตามมาตรฐาน `/follow-write-devin-skills`
...
1. เลือก prefix ตาม `/follow-write-devin-skills`: `run-*`, `follow-*`, `check-*`, `review-*`, `report-*`, `idea-*`, `lib-*`
...
1. แปลง tasks ใน session เป็น `## Execute` steps ตามมาตรฐาน `/follow-write-devin-skills`
...
1. ทำ `/follow-write-devin-skills` เพื่อเลือก template และสร้าง directory structure
...
- ทำตาม `/follow-write-devin-skills` สำหรับ structure และ template
...
- `SKILL.md` ถูกต้องตามมาตรฐาน `/follow-write-devin-skills` ไม่เกิน 250 บรรทัด
- ผ่าน `/validate`
