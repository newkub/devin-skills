...
16. ทำ `/review-codebase` เพื่อ review SRP violations, duplication, complexity, coupling, dead code, code smells, testability (dependency injection, pure functions, side effect isolation, test setup complexity)
17. ทำ `/review-codebase` เพื่อ review null safety, type assertions, exhaustive control flow, async/promise bugs
18. ทำ `/review-codebase` เพื่อ review deprecation policy, backward compatibility, migration guides, breaking changes, และ JSDoc/TSDoc completeness, inline comment quality, public API documentation (Section 2: Documentation Review)
19. ทำ `/review-codebase` เพื่อ review TODO, MOCK, STUB, placeholder, unfinished features, missing features (API/database มีแล้วแต่ UX/UI ยังไม่สมบูรณ์)
20. ทำ `/review-codebase` เพื่อ review framework, library, runtime compatibility, technology alignment, dependency versions, vulnerabilities, unused packages
21. ทำ `/review-codebase` เพื่อ review library design, API surface, bundle size, tree-shaking, peer deps, semver, export strategy (ถ้า project เป็น library)
22. ทำ `/review-codebase` เพื่อ review async/await, race conditions, deadlocks, parallel execution, shared state, timeout strategies
23. ทำ `/review-codebase` เพื่อ review error boundaries, try-catch coverage, unhandled rejections, error messages, error recovery
24. ทำ `/review-codebase` เพื่อ review cross-module config consistency, coding style, import/export patterns, folder structure, directory organization, module boundaries, import paths
