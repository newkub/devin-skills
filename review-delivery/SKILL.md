...
9. ทำ `/review-docs` เพื่อ deep documentation review เฉพาะทาง
...
Review deprecation ครอบคลุม policy, backward compatibility, migration guides, breaking changes

> Goal: ครอบคลุมทุก deprecation dimension

1. ตรวจสอบ deprecation policy, deprecation notices, และ deprecation timeline
2. ตรวจสอบ backward compatibility, compatibility layers, และ breaking change detection
3. ตรวจสอบ migration guides, migration tooling, และ migration path clarity
4. ตรวจสอบ version sunset, EOL communication, และ upgrade path
5. Critical: breaking change without notice, no migration path, silent API removal
6. High: missing deprecation notice, no migration guide, broken backward compat

### 12. Git Hygiene Review

Review git practices ครอบคลุม commit conventions, branch naming, PR size limits, merge strategy, gitignore

> Goal: ครอบคลุมทุก git hygiene dimension

1. ตรวจสอบ commit conventions: conventional commits, commit scope, breaking change notation
2. ตรวจสอบ branch naming: branch prefix, issue reference, naming consistency
3. ตรวจสอบ PR size: file count threshold, line count threshold, single concern enforcement
4. ตรวจสอบ merge strategy: squash vs merge vs rebase, merge commit message, fast-forward policy
5. ตรวจสอบ gitignore coverage: build artifacts, dependencies, secrets, editor files, OS files
6. ตรวจสอบ git hooks: pre-commit, pre-push enforcement, hook completeness
7. ตรวจสอบ git history: linear history, merge conflict patterns, stale branches
8. Critical: secrets in git history, no gitignore for sensitive files, no pre-commit hooks
9. High: inconsistent commit conventions, no branch protection, missing gitignore patterns
