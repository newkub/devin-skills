...
const renderer: RendererObject = {
  code({ text, lang }: Tokens.Code) {
    ...
    return `<pre class="hljs rounded-lg p-3 overflow-x-auto bg-background border border-border"><code class="language-${language}">${html}</code></pre>`
  },
}
...
export default function Markdown(props: MarkdownProps) {
  ...
  return (
    <div
      class={`markdown-body leading-relaxed text-muted-foreground ${props.class || ''}`}
      innerHTML={html()}
    />
  )
}
