...
const FeatureCard: Component<FeatureCardProps> = (props) => {
  ...
  return (
    <button
      ...
      class={`group w-full text-left flex flex-col gap-1.5 rounded-lg border bg-surface p-3 transition focus:outline-none ${
        props.selected
    ...
    >
      <div class="flex items-start justify-between gap-2">
        ...
        <span class="font-mono text-[10px] text-muted-foreground">#{f().number}</span>
      </div>

      <h3 class="text-sm font-semibold leading-snug text-surface-foreground">
        {f().feature}
      </h3>
      ...
      <div class="mt-auto flex flex-wrap items-center gap-1.5 pt-1">
        ...
        <Badge class="bg-muted text-surface-foreground text-[10px]">⭐ {f().mvpScore}</Badge>
        <Show when={(f().tags || []).length > 0}>
          {(f().tags || []).slice(0, 2).map(t => (
            <Badge class="bg-surface-elevated text-muted-foreground text-[10px]">{t}</Badge>
          ))}
        </Show>
      </div>
    </button>
