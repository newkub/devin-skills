...
สำหรับ CI platform เฉพาะจะส่งต่อ `/resolve-errors github-actions`
...
1. ถ้า GitHub Actions → ทำ `/resolve-errors github-actions [run-id]` แล้ว return ผล
...
3. ถ้าเป้น GitHub Actions fail → ทำ `/resolve-errors github-actions` ก่อนแก้ไข
4. ถ้าเป้น Cloudflare Worker fail → ทำ `/resolve-errors cloudflare-worker` ก่อน re-deploy
