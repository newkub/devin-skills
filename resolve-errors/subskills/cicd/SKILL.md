...
2. เรียก `/resolve-errors github-actions --repo <owner/repo>`
3. หา worker ทีตรงกับ repo name แล้วเรียก `/resolve-errors cloudflare-worker --worker <worker>`
4. หรือหา Cloudflare Pages project แล้วเรียก `/resolve-errors cloudflare --project <project>`
5. สรุปผลด้วย `/report`
...
2. ใช้ helper script `scripts/resolve-errors.ts` หรือทำตามขั้นตอนใน reference
3. ถ้า fail → `/resolve-errors github-actions` สำหรับ CI หรือ `/resolve-errors cloudflare-worker` สำหรับ CD
4. สรุปผลด้วย `/report`
