...
(merged from: improve-consistency)
...
- Naming ลงลึก (rename ข้าม codebase) → ส่งต่อ `/review-writing`
...
2. ใช้ `/scan-codebase` และ `search`/`use-astgrep` เก็บตัวอย่าง pattern ที่ทำสิ่งเดียวกัน
