...
description: เปิดไฟล์และ directory ใน web browser โดยใช้ open-web CLI จาก src/
...
เปิดไฟล์ หรือ directory ใน web browser โดยแปลงเป็น `file://` URL หรือ render เป็น HTML ก่อน แล้วใช้ `open-web` CLI จาก `src/`

## Scope

ใช้กับไฟล์ หรือ directory ที่ต้องการ preview ใน browser
รองรับ `file://` URL, ไฟล์ทั่วไป, markdown, และ code files
ต้อง build `open-web` CLI จาก `src/` ก่อนใช้งานครั้งแรก
...
4. ตรวจสอบ `open-files-in-web/src/` มี CLI source ครบ

### 2. Build Open-Web CLI

> Goal: build CLI จาก submodule

1. cd เข้า `open-files-in-web/src/`
...
1. ใช้ `bun open-files-in-web/src/dist/index.js <url>` หรือ `node open-files-in-web/src/dist/index.js <url>`
...
- `src/` เป็น git submodule ของ `open-web` project
- ถ้า update submodule → รัน `git submodule update --remote open-files-in-web/src` แล้ว commit
- ไม่แก้ไข code ใน `src/` ผ่าน skill นี้โดยตรง ให้ใช้ `/deep-refactor` หรือ `/refactor` ถ้าต้องการ

## Expected Outcome

- Browser เปิด target ตามที่ต้องการ
- `open-web` CLI ทำงานได้ใน `src/`
