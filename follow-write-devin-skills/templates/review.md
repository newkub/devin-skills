...
- ระบุ review target. อ่าน configs, dependencies. ทำ `/scan-codebase`. ถ้า web project → เพิ่ม `/run-dev`. ทำ `/deep-analyze`. ทำ `/update-review-cli-and-run` (เรียก `/update-rules` ภายใน). รัน `bunx ast-grep scan --inspect summary`. รัน `bun --filter tools-review-codebase review-codebase:json`. จับ findings พร้อม evidence
