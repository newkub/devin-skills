...
(merged from: improve-docs)

## Goal

แก้ไข documentation findings จาก `review-docs`, `review-docs`, `review-writing`, `review-docs` และ `review-docs` จน docs ถูกต้อง ครบถ้วน และตรงกับ code จริง
...
3. ถ้า findings เกี่ยวกับ nav/sidebar หรือ VitePress config → ทำ `/update-vitepress-docs` โดยใช้ `update-docs/references/<type>.md` และ `update-vitepress-docs/templates/sidebar-<type>.md` ตาม type (`product`, `open-source`, `cli`) บวก `update-vitepress-docs/templates/sidebar-monorepo.md` สำหรับ monorepo; ถ้า findings เกี่ยวกับ content/pages → ทำ `/update-docs`
