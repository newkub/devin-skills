...
5. ถ้า skill มี `src/` → ทำ `/ship-verify-cicd` เลยหลัง validation ผ่าน (ไม่ต้องถาม user)
...
- ถ้า skill มี `src/` → ทำ `/ship-verify-cicd` เลยหลัง validation ผ่าน ไม่ต้องถาม user
...
- ถ้า skill มี `src/` จะถูกแปลงเป็น submodule ผ่าน `/convert-to-git-submodules` และ ship ผ่าน `/ship-verify-cicd` เลย
