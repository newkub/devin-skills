...
- อ่าน `review-dependencies/references/techstack-catalog.md` เพื่อสรุป tech stack และทำ `/review-dependencies` เพื่อ review dependencies ถ้ามี dependencies
- ถ้าสร้าง CLI → `/follow-create-cli`; ถ้า website → `/follow-create-web`; ถ้า MCP server → `/follow-create-mcp` (Rust ก่อน แล้ว fallback TypeScript); ถ้า plugins/extensions → หา `follow-create-<target>-*` ทีมีอยู่ หรือทำ `/learn` (web) จาก official docs
