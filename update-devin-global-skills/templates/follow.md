...
- สร้าง/อัปเดท config ด้วย `/review-delivery`. ถ้ามี breaking changes → migration steps. ถ้าแก้ >10 ไฟล์ → ทำ `/use-scripts`. รัน typecheck, lint, tests. ถ้ามี errors → ทำ `/resolve-errors`. ถ้าผ่าน → ทำ `/ship-verify-cicd` สำหรับ project/app/CLI ที่สร้าง แล้ว `/suggest-next-action`
