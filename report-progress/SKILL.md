...
2. ใช้ `/report table` แสดง todo/roadmap โดย sort ตาม status ให้ `completed` อยู่บนสุด
...
- ใช้ `/report table` สำหรับรายการ todos โดย sort ตาม status: `completed` → `in_progress` → `pending` → `blocker`
