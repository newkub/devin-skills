...
| 12 | `bunx knip --workspace <pkg>` crash `RangeError: Array buffer allocation failed` ใน `oxc-parser` raw-transfer เมื่อ RAM เหลือน้อย | bun-packages `knip@6.35.1` + `oxc-parser@0.148.0` (packages/fx session, 2026-09-16) | open — environmental; workaround: เพิ่ม `--max-old-space-size` หรือปิด processes อื่นก่อนรัน |
