...
ใช้ก่อนเรียก `run-deploy`, `deploy-to-railway`, `deploy-to-cloudflare`, `follow-service-vercel`, `follow-service-vercel-cli`, `follow-deploy`, หรือ `run-preview` — ตรวจ deployment readiness ครอบคลุม env, secrets, build, health, rollback, DNS, SSL, migration, zero-downtime แล้วสรุป deploy readiness score พร้อม go/no-go checklist
...
2. ตรวจ secrets จัดการด้วย `/follow-secret-manager` ก่อน deploy ไป platform
