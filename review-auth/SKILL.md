---
name: review-auth
description: Review auth ครอบคลุม RBAC auth flows session token MFA OAuth SSO password reset JWT Supabase perm...
allowed-tools:
  - read
  - edit
  - grep
...
auth review สำหรับ: RBAC (roles, permissions, guards, enforcement), auth flows (login, logout, MFA, OAuth, SSO, password reset, email verification), session management, token/JWT management, Supabase auth integration, permission system, account linking — ไม่ทับซ้อนกับ `/review-codebase` ซึ่งครอบคลุม browser security, data leak, compliance, secrets management, rate limiting, webhooks, zero trust
