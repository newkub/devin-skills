...
description: อัปเดต rules ทั้งหมดใน project ทั้ง devin rules และ ast-grep rules พร้อมแปลงเป็น YAML
related:
  - analyze-project
  - update-dot-devin
...
อัปเดต rules ทั้งหมดใน project ทั้ง `.devin/rules/` (devin rules) และ `rules/` (ast-grep rules) พร้อมแปลง devin rules เป็น ast-grep YAML ที่ครอบคลุม atomic, relational, และ composite rules

## Scope

ใช้สำหรับอัปเดต rules ทั้งระบบใน project workspace ใดๆ ที่มี `.devin` structure ครอบคลุม devin rules, ast-grep rules และ `sgconfig.yml` setup

## Execute

### 1. Update Devin Rules

> Goal: อัปเดต devin rules ทั้ง 3 subdirectories ตาม project analysis และ dependencies จริง โดยจัดกลุ่ม `always-on/` เป็น `libs/`, `code-quality/`, `architecture/`
...
6. อัปเดต `libs/` — rules สำหรับ libraries และ tools จาก dependencies (เช่น `biome`, `bun`, `typescript`, `turborepo`, `lefthook`, `ast-grep`, `knip`, `vite`, `solidjs`, `unocss`, `vitest`, `drizzle`, `orpc`, `zod`, `supabase`, `stripe`, `playwright`, `capacitor`, `vitepress`, `mcp-sdk`): ตรวจสอบ config files, สร้างไฟล์ใหม่สำหรับ tool ใหม่, ลบไฟล์ tool ที่ไม่ใช้ ครอบคลุม commands, config locations, naming conventions, anti-patterns, integration points
7. อัปเดต `code-quality/` — rules สำหรับ code quality standards (เช่น `madge`, `knip`, code duplication, unused exports, circular dependencies): ครอบคลุม detection commands, thresholds, fix strategies, prevention patterns
8. อัปเดต `architecture/` — rules สำหรับ architecture และ project structure ของแต่ละ workspace (เช่น `architecture/website.md` สำหรับ `apps/website`, `architecture/docs.md` สำหรับ `apps/docs`): ครอบคลุม directory structure, module boundaries, import rules, export rules, workspace-specific patterns
9. อัปเดต `.devin/rules/model_decision/` — rules สำหรับ domain patterns (เช่น `api`, `auth`, `database`, `payment`, `deployment`, `mobile`, `testing`, `mcp`, `ai`, `i18n`, `realtime`, `seo`, `security`, `performance`): ครอบคลุม architecture, data flow, error handling, security, testing, integration points, anti-patterns
10. อัปเดต `.devin/rules/glob/` — rules สำหรับ file patterns (เช่น `**/schema/*.ts`, `**/server/*.ts`): อัปเดต `globs:` list ให้สอดคล้องกับ directory structure ปัจจุบัน
...
12. ลบ rules ที่ล้าสมัยหรือไม่ใช้แล้ว และอัปเดต references ทั้งหมด
...
> Goal: แปลง devin rules เป็น ast-grep YAML ครอบคลุมทุก devin rules ที่อัปเดตใน step 1
...
11. ลบ ast-grep rules ที่ล้าสมัยหรือไม่ใช้แล้ว และอัปเดต `sgconfig.yml` และ references ทั้งหมด
...
> Goal: อัปเดต `AGENTS.md`

อัปเดต `AGENTS.md` ให้สอดคล้องกับ rules ที่อัปเดต
...
3. ตรวจสอบว่า root และ workspace `AGENTS.md` อัปเดตครบถ้วน
...
- อัปเดต devin rules ก่อนเสมอ เพราะ ast-grep rules อ้างอิงจาก devin rules
- ลบ devin rules ที่ล้าสมัยหลังจากอัปเดตเสร็จ และอัปเดต references
- ทำ conversion หลังจาก devin rules อัปเดตเสร็จ ใช้ `/use-scripts` แทนเมื่อ ast-grep ไม่เหมาะสม
- ลบ ast-grep rules ที่ล้าสมัยหลังจากอัปเดตเสร็จ และอัปเดต `sgconfig.yml` และ references
...
- ถ้า devin rule เพิ่ม/เปลี่ยน/ลบ ให้ตรวจสอบและอัปเดต ast-grep rule ที่เกี่ยวข้องทันที
- ถ้า ast-grep rule เพิ่ม/ลบ ให้ตรวจสอบว่า `sgconfig.yml` และ scripts อัปเดตด้วย
...
- ถ้า dependency เปลี่ยน version ให้อัปเดต rule content ให้สอดคล้อง
...
- `AGENTS.md` อัปเดตครบถ้วนทั้ง root และ workspace level ด้วย `/update-agents-md`
