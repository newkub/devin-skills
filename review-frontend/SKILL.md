---
name: review-frontend
description: Orchestrator สำหรับ frontend review ครอบคลุม 17 sub-review workflows แบบ parallel
---

## Goal

Orchestrate frontend review ครอบคลุม 17 sub-review workflows แบบ parallel พร้อม validate findings

## Scope

ใช้สำหรับ frontend review ทั้งหมด — เรียก sub-review workflows โดยตรง ไม่ทำ review เอง — ไม่รวม backend, infrastructure, หรือ security reviews

## Execute

### 1. Prepare And Update Rules

เตรียม context และอัปเดต rules ก่อนรัน sub-reviews

> Goal: rules และ analyzers ครอบคลุมล่าสุด พร้อมรัน sub-reviews

1. ทำ `/update-codebase-health-cli` — `/update-codebase-health-cli` เรียก `/update-rules` ภายในเพื่ออัปเดต ast-grep rules
2. ถ้า `/update-codebase-health-cli` ข้าม `/update-rules` → ทำ `/update-rules` แยก
3. รัน `bunx ast-grep scan --inspect summary` เพื่อ verify rules ทำงานได้
4. ถ้าเป็น web project → เพิ่ม `/run-dev` เพื่อ verify dev server

### 2. Run Frontend Sub-Reviews

รัน 17 frontend sub-review workflows แบบ parallel

> Goal: ครอบคลุมทุก frontend dimension ผ่าน 17 sub-review workflows

1. ทำ `/review-uxui`, `/review-components`, `/review-hooks-composables`, `/review-form`, `/review-accessibility`, `/review-design-system`, `/review-responsive`, `/review-rendering`, `/review-css`
2. ทำ `/review-i18n`, `/review-images`, `/review-seo`, `/review-bundler`, `/review-browser-compat`, `/review-data-fetching`, `/review-event-handling`, `/review-state-management`
3. ถ้า sub-review ไม่เกี่ยวข้องกับ project → ข้าม sub-review นั้น
4. ถ้าพบ critical issues → หยุดและทำ `/validate` ก่อนดำเนินต่อ

### 3. Validate And Report

ตรวจสอบ findings และรายงานผล

> Goal: findings ถูก validate และรายงานเป็นตาราง

1. ทำ `/deep-validate` เพื่อ validate findings หลายมิติ
2. ทำ `/validate` สำหรับ validate issues จากทุก sub-review
3. จัดลำดับตาม severity: Critical → High → Medium → Low
4. คำนวณ health score: (Critical=0, High=25, Medium=50, Low=75, Info=100) → weighted average
5. ทำ `/report` พร้อม `/report-format-table`
6. ทำ `/suggest-next-action`

## Rules

### 1. Delegation

- Orchestrator เรียก sub-review workflows โดยตรง ไม่ทำ review เอง
- ไม่ duplicate เนื้อหา inline — Skip conditions ของแต่ละ workflow อยู่ใน workflow เอง
- ถ้า project ไม่มี dimension ใด → ข้าม sub-review นั้น

### 2. Update Before Run

- ทำ `/update-codebase-health-cli` ก่อนรัน sub-reviews เสมอ — `/update-codebase-health-cli` เรียก `/update-rules` ภายใน
- ถ้า `/update-codebase-health-cli` ข้าม `/update-rules` → ทำ `/update-rules` แยก

### 3. Formatting

- ห้ามใช้ `**` (bold markers) — ใช้ backticks สำหรับ emphasis
- รายงานเป็นตารางด้วย `/report-format-table`

## Expected Outcome

- Findings และ recommendations จาก 17 frontend sub-review workflows
- Issues ที่พบถูก validate ครบถ้วนตาม severity
- Health score ต่อ dimension และ overall
- รายงานในแชทเป็นตารางตาม `/report-format-table`
- แนะนำ action ถัดไปผ่าน `/suggest-next-action`
