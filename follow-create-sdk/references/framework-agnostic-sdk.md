...
กำหนด `exports` field ใน `package.json` เป็น subpath exports:
...
- Build ด้วย library bundler ที่รองรับ multi-entry เช่น `tsdown`, `tsup` หรือ `unbuild` (ดู `/follow-tool-tsdown`)
- ถ้า adapters ใหญ่หรือมี dependencies ต่างกันมาก → แยกเป็นหลาย packages (`@scope/name-core`, `@scope/name-react`) ใน monorepo ตาม `/follow-monorepo`
