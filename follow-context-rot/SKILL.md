...
> Goal: ลด context rot และปรับปรุงการจัดการ context

1. ตรวจจับสัญญาณ context rot และ mitigate ทันที: การตอบซ้ำ, ลืม context, หลุดจาก goal
2. สรุป goal, progress, tasks ที่เหลือ และบันทึกลง `progress.txt` หรือ `create_memory`
3. แนะนำให้เริ่ม conversation ใหม่เมื่อ context ใหญ่เกินไป ส่งสรุปเป็น context เริ่มต้น
4. ใช้ goal reminders ทุก 5-10 tool calls เพื่อลด context drift
5. แบ่ง long-horizon tasks เป็น sub-tasks ที่ทำเสร็จใน 5-10 tool calls ด้วย `/deep-plan` และ `/loop-until-complete`
6. จัดการ context window: ใช้ `offset`, `limit`, `code_search`, `OutputCharacterCount`, รวม parallel calls
