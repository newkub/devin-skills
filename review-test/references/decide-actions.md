...
1. ถ้ามี assertion/implementation failure → แนะนำ `update-tests` หรือ `deep-debug`
2. ถ้ามี runtime/setup failure → แนะนำ `resolve-errors` หรือ `update-config`
3. ถ้ามี coverage gap → แนะนำ `update-tests`
...
6. ถ้าพบว่า skill ที่ใช้ (เช่น `update-tests`, `follow-test`) ยังไม่ครอบคลุมกรณีที่เจอ → บันทึก gap
