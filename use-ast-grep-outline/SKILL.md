...
- ไม่ normalize extends, implements, traits, protocols
- ไม่ build cross-language relationship graph
- ไม่ resolve imports, follow references, สร้าง call graphs, หรือ infer receiver types
- ไม่ infer membership จาก names, receiver types, หรือ type resolution
- เป็น local-only: ไม่มี cross-file analysis
- ไม่ build index: parse on demand ทุกครั้ง
- Flat source ยังคง flat: Go receiver methods และ Rust impl blocks เป็น top-level entries
