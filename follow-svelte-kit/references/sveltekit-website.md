# Website

## Official Resources

| Resource | URL |
|----------|-----|
| Documentation | https://svelte.dev/docs/kit |
| Tutorial | https://svelte.dev/tutorial/kit |
| Playground | https://svelte.dev/playground |
| GitHub | https://github.com/sveltejs/kit |
| bun | https://www.bunjs.com/package/@sveltejs/kit |
| Discord | https://svelte.dev/chat |

## Quick Links

- [SvelteKit Docs](https://svelte.dev/docs/kit) - Official documentation
- [Svelte Tutorial](https://svelte.dev/tutorial) - Interactive tutorial
- [Svelte REPL](https://svelte.dev/repl) - Try Svelte online
- [SvelteKit Examples](https://github.com/sveltejs/kit/tree/main/examples) - Example projects

## Adapters

| Adapter | Package | Docs |
|---------|---------|------|
| Auto | `@sveltejs/adapter-auto` | Built-in |
| Node | `@sveltejs/adapter-node` | [Link](https://kit.svelte.dev/docs/adapter-node) |
| Static | `@sveltejs/adapter-static` | [Link](https://kit.svelte.dev/docs/adapter-static) |
| Vercel | `@sveltejs/adapter-vercel` | [Link](https://kit.svelte.dev/docs/adapter-vercel) |
| Cloudflare | `@sveltejs/adapter-cloudflare` | [Link](https://kit.svelte.dev/docs/adapter-cloudflare) |
| Deno | `@sveltejs/adapter-deno` | [Link](https://github.com/YanDge/deno-sveltekit-adapter) |
| Netlify | `@sveltejs/adapter-netlify` | [Link](https://kit.svelte.dev/docs/adapter-netlify) |

## Related Libraries

| Library | Description |
|---------|-------------|
| [Svelte](https://svelte.dev) | UI framework |
| [Vite](https://vitejs.dev) | Build tool |
| [TypeScript](https://www.typescriptlang.org) | Type safety |
| [Drizzle](https://orm.drizzle.team) | Database ORM |
| [Prisma](https://prisma.io) | Database ORM |
| [Lucia](https://lucia-auth.com) | Auth framework |
| [Zod](https://zod.dev) | Schema validation |

## Community

- [Discord](https://svelte.dev/chat) - Chat with community
- [Reddit](https://reddit.com/r/sveltejs) - Svelte subreddit
- [Twitter](https://twitter.com/sveltejs) - Official Twitter
- [GitHub Discussions](https://github.com/sveltejs/kit/discussions) - Q&A

## Version

Current stable version: Check [bun](https://www.bunjs.com/package/@sveltejs/kit) for latest
