...
ดู `references/workspace-package.md` สำหรับ workspace package validation rules
...
2. ตรวจว่าทุก analyzer มี `reviewWorkflow` map ไปยัง `/review-codebase` references
3. ตรวจว่า analyzers deterministic: ใช้ `git grep` และ `walk` ที่ stable
4. ตรวจว่า findings ระบุ file/line เสมอ
5. บันทึก findings พร้อม evidence

ดู `references/analyzers.md` สำหรับ analyzer structure และ behavior rules
...
ดู `references/exports.md` สำหรับ workspace API exports validation rules
...
ดู `references/review-integration.md` สำหรับ integration validation rules
...
ดู `references/index.md` สำหรับ reference files mapping
