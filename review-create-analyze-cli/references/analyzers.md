...
ทุก analyzer ต้องมี `reviewWorkflow` map ที่ map finding type ไปยัง `/review-codebase` reference:

```typescript
const reviewWorkflow: Record<string, string> = {
  "missing-eslint": "review-codebase/lint-rules",
  "no-tests": "review-codebase/test-coverage",
};
```
