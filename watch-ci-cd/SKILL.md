---
name: watch-ci-cd
description: ติดตาม CI/CD pipeline ทุก platform จนผ่าน และแก้ไข failures
...
ติดตามสถานะ CI/CD pipeline ตรวจจับ failures และแก้ไขจนกว่าจะผ่าน

## Scope

ใช้หลัง push code หรือเมื่อได้รับ `run-id` จาก argument ครอบคลุม GitHub Actions, GitLab CI, Azure DevOps, CircleCI, Jenkins, Cloudflare Pages และ Vercel ผ่าน CLI หรือ API
...
> Goal: ระบุ CI/CD platform ของ repository ปัจจุบัน
...
6. Cloudflare: รัน `wrangler --version`
7. Vercel: รัน `vercel --version`
8. ถ้า CLI ไม่พร้อมหรือไม่ authenticate → stop และ report สิ่งที่ต้อง install หรือ login

### 3. Identify Latest Run

> Goal: หา pipeline run ล่าสุดที่จะติดตาม
...
7. Cloudflare Pages: `wrangler pages deployment list`
8. Vercel: `vercel list`
9. ถ้าไม่มี active run → report และจบ task

### 4. Watch Pipeline Until Complete

> Goal: ติดตาม pipeline จนกว่าจะสิ้นสุด
...
6. Cloudflare Pages: รัน `/watch-cloudflare`
7. Vercel: รัน `/watch-vercel`
8. ถ้า pipeline ผ่าน → ไปขั้นตอน 7
9. ถ้า pipeline ล้มเหลว, ถูก cancel หรือ timeout → ไปขั้นตอน 5
...
7. บันทึก root cause และ file path, line number ถ้ามี
...
- ตรวจหา config files ก่อนเดา platform
...
- วน loop จนกว่า pipeline จะผ่านหรือถึงขีดจำกัดรอบ
...
- ใช้ sub-skill ที่เหมาะสม: `/watch-build`, `/watch-test`, `/watch-lint`, `/watch-deploy`
...
- ห้าม commit code ขณะที่ pipeline ยังไม่ผ่าน
...
- CI/CD pipeline ผ่านสำเร็จโดยไม่มี failure เหลือ
