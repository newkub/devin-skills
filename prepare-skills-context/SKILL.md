...
ใช้เมื่อจะสร้างหรือปรับปรุง skill files ใน `global skills` หรือ workspace — ไม่ซ้ำกับการเขียนเนื้อหา skill (`/follow-create-devin-skills`) หรือการ validate (`/validate`)
...
1. เลือก template ตาม prefix:
   - `run-*` → `follow-create-devin-skills/templates/run.md`
   - `follow-*` → `follow-create-devin-skills/templates/follow.md` ยกเว้น `follow-*-architecture` → `follow-create-devin-skills/templates/follow-architecture.md`
   - `check-*` → `follow-create-devin-skills/templates/check.md`
   - `analyze-*` → `follow-create-devin-skills/templates/analyze.md`
   - `deep-*` → `follow-create-devin-skills/templates/deep.md`
   - `review-*` → `follow-create-devin-skills/templates/review.md`
   - `idea-*` → `follow-create-devin-skills/templates/idea.md`
   - `report-*` → `follow-create-devin-skills/templates/report.md`
   - ถ้าไม่ตรง → บันทึกว่าไม่มี template ตรง prefix
2. อ่าน template ที่เลือกเพื่อดูโครงสร้าง sections, steps, และ rules ขั้นต่ำ
   - ใช้ relative path `follow-create-devin-skills/templates/<prefix>.md`
3. ถ้าไม่มี template ตรง prefix → ระบุใน report และให้ `/suggest-next-action` แนะนำ `/follow-create-devin-skills`
...
- ใช้ templates ใน `follow-create-devin-skills/templates/*.md` เป็น canonical structure
...
- `review-*`/`analyze-*` → ใช้ `/update-review-codebase-cli-and-run` สำหรับ analysis, มี severity, actionable recommendations, output เป็น `/report-table`
...
- `improve-*`/`write-*` → ใช้ `/follow-create-devin-skills` เป็นมาตรฐาน, มี review ก่อน improve
...
- ถ้า template ไม่ตรง → ให้ `/suggest-next-action` แนะนำ next step เช่น `/follow-create-devin-skills`
