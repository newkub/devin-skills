...
ไม่รวม:
- สร้าง UI component ตาม framework (ให้ใช้ `follow-react`, `follow-nextjs`, ฯลฯ)
...
1. ติดตั้ง client SDK ตาม framework:
   ...
   - Python: `pip install instantdb` (ใช้งานร่วมกับ `follow-python`)
...
1. อ่าน `references/api.md` และ `references/configuration.md`
...
5. ดู patterns เพิ่มเติมใน `references/api.md`

### 7. Setup Auth And Permissions
> Goal: ป้องกันข้อมูลด้วย auth และ rules

1. ถ้าใช้ built-in auth: อ่าน `references/configuration.md` เรื่อง magic codes, OAuth
...
1. ถ้ามีการเพิ่ม links/rooms → อัปเดต `references/configuration.md`
