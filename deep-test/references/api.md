# Deep Test — API Domain

รัน API tests โดยเรียก endpoints จริงหรือ mocked แล้วตรวจสอบ responses เพื่อ verify contracts และ behavior — ครอบคลุม REST, GraphQL, tRPC, WebSocket
...
## 5. All-Routes Check (merged จาก test-all-api-routes)

> Goal: ยิงทุก API route ของ domain — status + response time + severity ต่อ route

ใช้เมื่อต้องการ coverage ครบทุก route (ไม่ใช่แค่ assertions บางจุด) — safe by default: ยิงเฉพาะ `GET`/`HEAD`/`OPTIONS`, mutation methods ต้อง `--allow-write` + user confirmation

1. Resolve route source ตามลำดับ: `crw_map` (MCP) → `--routes-file` → OpenAPI spec paths → `--discover` (SPA same-origin links) → `/report-uxui-all-routes` จาก source code; ถ้าหาไม่ได้เลย → stop และ report
2. รัน route checker script:
   ```bash
