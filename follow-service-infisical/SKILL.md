...
- ระบุ environment: `infisical run --env=dev -- bun run dev`
- Auto-reload: `infisical run --watch -- bun run dev`
- กรองตาม path: `infisical run --path="/api" -- bun run dev`
- กรองตาม tags: `infisical run --tags=tag1,tag2 -- bun run dev`
- หลาย commands: `infisical run --command="bun run build && bun run start"`
- ถ้า project ใช้ npm เป็นหลัก → แทนด้วย `npm run`
...
- ดูรายละเอียดการตั้งค่าใน [references/platform-integrations.md](references/platform-integrations.md)
...
### 7. Monorepo Scripts Integration

เพิ่ม scripts ที่ root `package.json` สำหรับ Infisical integration กับ Turborepo

- Root scripts:
  - `"secrets:dev": "infisical run --env=dev -- turbo run dev"` — dev พร้อม secrets injection
  - `"secrets:build": "infisical run --env=prod -- turbo run build"` — build พร้อม production secrets
