# Website

## Official Resources

| Resource | URL |
|----------|-----|
| Documentation | https://arktype.io/ |
| GitHub | https://github.com/arktypeio/arktype |
| bun | https://www.bunjs.com/package/arktype |
| Discord | https://discord.gg/arktype |

## Quick Links

- [Playground](https://arktype.io/playground) - Try ArkType online
- [GitHub Repository](https://github.com/arktypeio/arktype) - Source code
- [Changelog](https://arktype.io/changelog) - Version history

## Community

- [Discord Community](https://discord.gg/arktype) - Chat with users
- [GitHub Issues](https://github.com/arktypeio/arktype/issues) - Report bugs
- [GitHub Discussions](https://github.com/arktypeio/arktype/discussions) - Ask questions

## Related Libraries

| Library | Description |
|---------|-------------|
| [Zod](https://zod.dev) | Alternative validation library |
| [Valibot](https://valibot.dev) | Lightweight validation |
| [Yup](https://github.com/jquense/yup) | Object schema validation |

## Version

Current stable version: Check [bun](https://www.bunjs.com/package/arktype) for latest
