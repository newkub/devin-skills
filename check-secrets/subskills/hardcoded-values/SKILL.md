---
name: check-secrets-hardcoded-values
...
- Read-only: รายงานอย่างเดียว — ไม่แยกกับ `/check-secrets secrets-leak` ซึ่งจัดการ credentials จริง
...
3. ข้าม secrets patterns → ส่งต่อ `/check-secrets secrets-leak`
...
3. แนะนำ `/check-secrets env-vars` หลังย้ายค่าไป env
...
- ถ้าเจอค่าที่เป็น credential จริง → ส่งต่อ `/check-secrets secrets-leak` ไม่ report ค่าเต็มที่นี่

### 3. Read-Only

- ไม่แก้ code หรือสร้าง config — รายงานพร้อมแนะนำ `/follow-config`

- ใช้ /check-secrets secrets-leak สำหรับ credentials จริง
- ใช้ /check-secrets env-vars ตรวจ env keys หลัง refactor
